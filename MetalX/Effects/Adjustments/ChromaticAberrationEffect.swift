    ...
    override func apply(to texture: MTLTexture, commandBuffer: MTLCommandBuffer, device: MTLDevice) -> MTLTexture? {
        ...
        if pipelineState == nil {
            ...
            do {
                ...
                print("Failed to create chromatic aberration pipeline state: \(error)")
                return texture
            }
        }
        ...
        // Set parameters
        var params = simd_float4(redOffset * intensity, blueOffset * intensity, Float(texture.width), Float(texture.height))
